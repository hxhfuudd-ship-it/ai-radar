...
function compactPromptText(value: string | null | undefined, maxChars: number) {
  if (!value) return '';
...
async function chatAboutProject(
  ...
  const summaryText = compactPromptText(project.summary, 220);
  const analysisText = compactPromptText(project.analysis, 260);
